...
public class FourSquareCipher implements Cipher
{
    ...
    private List<List<Character>> cipher ;
    
    public FourSquareCipher(String key1, String key2)
    {
        key1 = key1.toLowerCase().replaceAll("[^a-zA-Z0-9 ]", "").replaceAll("j", "i");
        key2 = key2.toLowerCase().replaceAll("[^a-zA-Z0-9 ]", "").replaceAll("j", "i");
        cipher = new ArrayList<List<Character>>()
                {{
    ...
    {
        Cipher cc = new FourSquareCipher("battle", "shipps");
        String message = "my favourite thing is 123";
        String encoded = cc.encrypt(message);
        System.out.println(encoded);
        String decoded = cc.decrypt(encoded);
    ...
    {
        message = message.toLowerCase().replaceAll("[^a-zA-Z0-9 ]", "").replaceAll("j", "i");
        if((message.length()&1)==1)
            message += ALPHABET.charAt((int)(Math.random()*ALPHABET.length()));
        StringBuilder sb = new StringBuilder(message.length());
        // removed next line -- reimplement if we want to be a tad bit more efficient
        // Character extra = (message.length()&1)==1 ? ALPHABET.charAt((int)(Math.random()*ALPHABET.length())) : null;
        // linear search in case we want to swap out alphabet ie caesar shift the alphabet etc
